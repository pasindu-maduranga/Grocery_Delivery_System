...
export default function SuppliersPage() {
  ...
  // ── Reject ───────────────────────────────────────────────────────
  const openReject = (supplier) => { setRejectNote(''); setActionError(''); setRejectModal({ open: true, supplier }) }
    ...
    <Layout title="Suppliers" subtitle="Supplier Management">
      <div className="card">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h3 className="font-semibold text-slate-800">All Suppliers</h3>
            ...
            <tr key={s._id} className="hover:bg-slate-50 transition-colors">
              ...
              <td className="table-cell">
                <div className="flex flex-wrap gap-1 max-w-[180px]">
                  ...
                  {s.supplyCategories?.length > 3 && <span className="text-xs text-slate-400">+{s.supplyCategories.length - 3} more</span>}
                </div>
              </td>
              {/* Status */}
              ...
              <td className="table-cell">
                <div className="flex items-center gap-1">
                  <button onClick={() => navigate(`/suppliers/edit/${s._id}`)} title="Edit" className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-slate-100 text-slate-500">
                    <Pencil size={14} />
                  </button>
                  {s.status === 'pending' && <>
                    <button onClick={() => openApprove(s)} title="Approve" className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-emerald-50 text-emerald-600">
                      <CheckCircle size={14} />
                    </button>
                    <button onClick={() => openReject(s)} title="Reject" className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-red-50 text-red-400">
                      <XCircle size={14} />
                    </button>
                  </>}
                  {s.status === 'approved' && <>
                    <button onClick={() => setConfirm({ open: true, type: 'active', item: s })} title={s.isActive ? 'Deactivate' : 'Activate'} className={`w-8 h-8 flex items-center justify-center rounded-lg ${s.isActive ? 'hover:bg-red-50 text-red-400' : 'hover:bg-emerald-50 text-emerald-500'}`}>
                      <Power size={14} />
                    </button>
                    <button onClick={() => setConfirm({ open: true, type: 'lock', item: s })} title={s.isLocked ? 'Unlock' : 'Lock'} className={`w-8 h-8 flex items-center justify-center rounded-lg ${s.isLocked ? 'hover:bg-emerald-50 text-emerald-500' : 'hover:bg-amber-50 text-amber-500'}`}>
                      <Lock size={14} />
                    </button>
                  </>}
                </div>
      ...
      {/* ── Approve Modal ─────────────────────────────────────── */}
      <Modal open={approveModal.open} onClose={() => setApproveModal({ open: false })} title="Approve Supplier & Set Portal Access" size="md">
      ...
      {/* ── Reject Modal ──────────────────────────────────────── */}
      <Modal open={rejectModal.open} onClose={() => setRejectModal({ open: false })} title="Reject Supplier Application" size="sm">
        <form onSubmit={handleReject} className="p-6 space-y-4">
          {actionError && <p className="text-sm text-red-500 bg-red-50 border border-red-200 px-3 py-2 rounded-lg">{actionError}</p>}
          <p className="text-sm text-slate-600">Rejecting <strong>{rejectModal.supplier?.businessName}</strong>. An email will be sent to notify them.</p>
